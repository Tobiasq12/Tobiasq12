...
function TornadoVortex:TornadoVortex(initialPosition)
    ...
    self._destination = GetRandomPositionFromCoords(initialPosition, 200.0)
...
function TornadoVortex:Build()
    ...
    for i = 0, maxLayers, 1 do
        for angle = 0, particleCount, 1 do
            ...
            --     sound:GameSound("BRUSHES_SPINNING", "CARWASH_SOUNDS")
    ...
    -- local particle = TornadoParticle:new()
    -- particle:TornadoParticle(self, self._position, vec3(0.0, 0.0, 0.0), "des_stilthouse", "ent_ray_fam3_dust_linger", radius, -1)
    -- particle:StartFx(12.0)
    -- table.insert(self._particles, particle)
end
...
function TornadoVortex:CollectNearbyEntities(maxDistanceDelta)
    ...
    local function TryAddEnt(entity)
        if #(pos - GetEntityCoords(entity)) < maxDistanceDelta then
            local found = false
...
function TornadoVortex:OnUpdate(gameTime)
    if VORTEX_MOVEMENT_ENABLED then
        ...
        if GetDistanceBetweenCoords(self._position, self._destination) < 15.0 then
            ...
            self._destination = GetRandomPositionFromCoords(self._position, 200.0)
        end
